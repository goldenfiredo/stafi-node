...
// use stafi_service as stafiservice;
...
impl AugmentClap for StafiCustom {
	fn augment_clap<'a, 'b>(app: App<'a, 'b>) -> App<'a, 'b> {
		Self::augment_clap(app)
	}
}

impl GetLogFilter for StafiCustom {
    fn get_log_filter(&self) -> Option<String> {
        None
    }
}

fn load_spec(id: &str) -> Result<Option<stafi_service::chain_spec::ChainSpec>, String> {
...
fn read_args_from_config(config_path: &str) -> Option<Vec<String>> {
    let contents =
        std::fs::read_to_string(config_path).expect("The configuration file is invalid!");
                ...
                match result {
                    Ok(s) => {
                        if name.starts_with("--") {
                            ...
                            args.push(("--".to_owned() + &name).to_owned());
                        }
                        if let Some(s) = s {
                            args.push(s);
                        }
